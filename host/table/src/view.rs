...
impl<F: File<Key = NodeId, Block = Node>, D: Dir, Txn: Transaction<D>> IndexSlice<F, D, Txn> {
...
impl<F: File<Key = NodeId, Block = Node>, D: Dir, Txn: Transaction<D>, T: TableInstance>
    Selection<F, D, Txn, T>
{
...
impl<F: File<Key = NodeId, Block = Node>, D: Dir, Txn: Transaction<D>> Default
    for Phantom<F, D, Txn>
{
    fn default() -> Self {
        Self {
