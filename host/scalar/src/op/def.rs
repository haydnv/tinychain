...
use tc_transact::public::{
    DeleteHandler, GetHandler, Handler, PostHandler, PutHandler, StateInstance,
};
use tcgeneric::*;

use crate::{Executor, Refer, Scalar, TCRef};
...
impl OpDef {
    ...
    {
        match self {
            Self::Get((key_name, form)) => {
                Self::Get((key_name, dereference_self::<State>(form, path)))
            }
            Self::Put((key_name, value_name, form)) => {
                Self::Put((key_name, value_name, dereference_self::<State>(form, path)))
            }
            Self::Post(form) => Self::Post(dereference_self::<State>(form, path)),
            Self::Delete((key_name, form)) => {
                Self::Delete((key_name, dereference_self::<State>(form, path)))
            }
        }
    }
        ...
        Scalar: Refer<State>,
    {
        self.form()
            ...
            .any(|provider| Refer::<State>::is_inter_service_write(provider, cluster_path))
    }
        ...
        Scalar: Refer<State>,
    {
        match self {
            Self::Get((key_name, form)) => {
                Self::Get((key_name, reference_self::<State>(form, path)))
            }
            Self::Put((key_name, value_name, form)) => {
                Self::Put((key_name, value_name, reference_self::<State>(form, path)))
            }
            Self::Post(form) => Self::Post(reference_self(form, path)),
            Self::Delete((key_name, form)) => {
                Self::Delete((key_name, reference_self::<State>(form, path)))
            }
        }
    }
}
            ...
            Some(Box::new(|txn, key| {
                Box::pin(async move {
                    ...
                    let data = iter::once((key_name, key)).chain(
                        op_def
                            .into_iter()
                            .map(|(id, provider)| (id, State::from(provider))),
                    );

                    let executor: Executor<State, State> = Executor::new(txn, None, data);
...
fn dereference_self<State>(form: Vec<(Id, Scalar)>, path: &TCPathBuf) -> Vec<(Id, Scalar)>
where
    State: StateInstance + Refer<State> + From<Scalar>,
    TCRef: Refer<State>,
    Scalar: Refer<State>,
...
fn reference_self<State>(form: Vec<(Id, Scalar)>, path: &TCPathBuf) -> Vec<(Id, Scalar)>
where
    State: StateInstance + Refer<State> + From<Scalar>,
    Scalar: Refer<State>,
{
