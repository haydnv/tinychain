...
impl<T> SyncChain<T>
where
    T: Persist<CacheBlock, Txn = Txn> + Restore<CacheBlock> + TryCastFrom<State>,
{
    pub(crate) async fn restore_from(&self, txn: &Txn, source: Link) -> TCResult<()> {
...
#[async_trait]
impl<T: AsyncHash + Send + Sync> AsyncHash for SyncChain<T> {
