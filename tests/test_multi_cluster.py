...
class Left(Balance):
    ...
    def weigh(self, txn, key: tc.Nil, new_value: tc.Number):
        ...
        txn.update = tc.After(right.weigh(None, txn.total - new_value), self.weight.set(new_value))
...
class Right(Balance):
    ...
    def weigh(self, txn, key: tc.Nil, weight: tc.Number):
        ...
        txn.total = CONSERVED
        txn.current = self.weight.subject()
        txn.update = tc.After(left.weigh(None, txn.total - new_value), self.weight.set(new_value))
        return tc.If(txn.current == new_value, None, txn.update)


class InteractionTests(unittest.TestCase):
    def testStartup(self):
        host = start_host("left", [Left])
