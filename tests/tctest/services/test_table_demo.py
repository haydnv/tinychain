...
class TableDemoTests(unittest.TestCase):
    def setUp(self):
        ...
        for i in range(3):
            ...
            host = start_host("table_demo", [], True, host_uri, replicate=LEAD)
            self.hosts.append(host)

        self.hosts[0].put(tc.URI(tc.app.Service), str(web.NS)[1:], tc.URI(web)[:-2])
